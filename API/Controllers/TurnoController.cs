...
namespace API.Controllers
{
    public class TurnoController : BaseController
    {
        ...
        public async Task<ActionResult<TurnoDto>> Put(int id, [FromBody] TurnoDto resultDto)
        {
            ...
            if (resultDto.HorTurnoI == TimeOnly.MinValue)
            {
                exists.HorTurnoI = TimeOnly.FromDateTime(DateTime.Now);
            }
            if (resultDto.HorTurnoF == TimeOnly.MinValue)
            {
                exists.HorTurnoF = TimeOnly.FromDateTime(DateTime.Now);
            }
            await _unitOfWork.SaveAsync();
